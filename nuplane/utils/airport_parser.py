...
def download_airport_scenary_pack(airport_id, export_path):
...
def parse_airport_route_network(airport_id):
    export_path = f"data/{airport_id}/"

    # Dowload airport data
    download_airport_scenary_pack(airport_id, export_path)
    ...
    G = nx.MultiGraph()
    G.graph["crs"] = routes.crs
    key_id = 0
    for line in routes["geometry"].values:
...
def parse_airport_network(airport_id, feature="pavements"):
    export_path = f"data/{airport_id}/"
    ...
    # Read to Geopandas
    df = gpd.read_file(export_path + f"{airport_id}.{feature}.shp")

    if feature == "pavements":
        df = df[df["surface_ty"] == "CONCRETE"]
        return df
